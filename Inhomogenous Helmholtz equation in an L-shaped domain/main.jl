using SparseArrays, GLMakie, Plots;
import PrettyTables;
...
s = 0.05;
k = 5.0; # If k = 0 then we have the Poisson Equation (Because we have division by zero errors use eps(Float64) instead of 0). The bigger k is we have more peaks
...
@time "Building matrices time" begin
...
MGlevels = 2;
# sub1_lvls, sub2_lvls, sub3_lvls = buildMGlevels(step1, MGlevels, f, k, H);
# all_subR, all_subP = build_all_SubRnP(sub1_lvls, sub2_lvls, sub3_lvls);
nPre = 2;
nPost = 1;
omega = 4/5;
gamma = 1;
...
@time "Solving time" begin
    ...
    # u = MG_PCG(K1, F_global1, tol, idx, 500, # K1, F_global1 are for the global L-shape problem
    #            sub1_lvls, sub2_lvls, sub3_lvls, all_subR, all_subP, # These are for the preconditioner's MG sub-solves
...
# plot_fem_results(GO, all_coords, u, step1, omega, T, frames);
